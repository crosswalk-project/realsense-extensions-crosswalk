...
(function(exports) {
  ...
  function main(spDom) {
    ...
    if (spDom.spState < 0) {
      ...
      var leftView = new spReconstructionSample.LeftRender(sp, spDom);
      var rightView = new spReconstructionSample.RightRender(sp, spDom);
    ...
    function displayText(quality, accuracy) {
      ...
      if (spDom.spState == 1) {
        ...
        if (quality >= MIN_ACCEPTABLE_QUALITY) {
          ...
          leftHint.innerText =
              'Point to an area with more structure (few planar surfaces) to start.';
          leftHint.style.color = 'red';
        }
      }
